...
if __name__ == '__main__':
    ...
    #print(config['prepro'])
    #print(config['val_method'])
    #print(tune_log_path + 'best_params_'+config['loss_type']+'_'+config['algo_name']+'_'+config['dataset']+'_'+config['prepro']+'_'+config['val_method']+'.csv')
    ...
    f = open(
        tune_log_path + 'best_params_'+config['loss_type']+'_'+config['algo_name']+'_'+config['dataset']+'_'+config['prepro']+'_'+config['val_method']+'_'+config['path'][:-4]+'.csv',
        'w', encoding='utf-8')
    ...
    print('number of interactions in training data: ', len(train_set))
    # get rated data
    ...
    def objective(trial):
        ...
        for param in tune_param_names:
            ...
            if isinstance(param_dict[param], list):
                ...
                if param_type_config[param] == 'int':
                    ...
                    raise ValueError('Invalid parameter type for' +param+'...')
            else:
                raise ValueError('Invalid parameter settings for' +param+', Current is'+param_dict[param]+'...')
        ...
        for train_index, val_index in splitter.split(train_set):
            ...
            if config['algo_name'].lower() in ['itemknn', 'puresvd', 'slim', 'mostpop', 'ease']:
                ...
                model = model_config[config['algo_name']](config)#.to(config['device'])
            ...
            logger.info('Finish {cnt} train-validation experiment(s)...')
            cnt += 1
        ...
        TRIAL_CNT += 1
        logger.info('Finish'+str(TRIAL_CNT)+' trial...')
        print(np.mean(kpis))
        return np.mean(kpis)
    ...
    line = ','.join([str(study.best_params[param]) if param in param_dict.keys() else str(config[param]) for param in tune_param_names]) + ', '+str(study.best_value) +'\n'
    f.write(line)
    f.flush()
    f.close()
